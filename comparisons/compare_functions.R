...
# Sectoral maps??
  ...
  l_pdf_params <- list(species = pollutant,
                       plot_dir = plot_dir,
                       emis_yr_1 = emis_yr_1, emis_yr_2 = emis_yr_2,
                       folname_1 = folname_1, folname_2 = folname_2,
					   l_fname_1 = l_fname_1, l_fname_2 = l_fname_2,
					   l_m_1 = l_m_1, l_m_2 = l_m_2,
					   uk_plot1 = gg_p1,
					   uk_plot2 = gg_p2,
					   uk_plot3 = gg_p3,
					   uk_plot4 = gg_p4,
  ...
  rmarkdown::render(input = "comparisons/compare_markdown.Rmd", 
...
worked_totals <- function(pollutant, folname, fname, dt_metadata){
  ...
  for(a in c("GB", "IE", "SEA")){ # 2025+ data has GB, not UK
	...
	if(nc_time_len == 1){
	  ...
	  # quick tots check
	  sumsec <- rbindlist(l_summary, use.names = T)[Area == a_adj, sum(emis_t, na.rm=T)]
	  summon <- rbindlist(temp_summary, use.names = T)[,sum(emis_t, na.rm=T)]
...
comp_map_sec_ann <- function(pollutant, l_m_1, l_m_2, plot_dir){
  ...
  for(i in 1:12){
    ...
    p_tots <- ggplot()+
      geom_spatraster(data = s_rc, na.rm = T)+	
	  {if(sum(global(s,sum,na.rm=T)[,1])>0)scale_fill_brewer(labels = v_labs, palette = "Spectral", breaks = 1:length(v_q), direction = brew_d)}+
	  scale_y_continuous(expand = c(0, 0)) +
	  scale_x_continuous(expand = c(0, 0)) +
	  # ggtitle(paste0(unique(l_m_1$summary$fname)," vs ",unique(l_m_2$summary$fname)))+
	  ggtitle(sec_GNFR)+
    ...
    # create a reclassification matrix and labels for the plot.
    m <- matrix(c(v_q, v_q[2:length(v_q)], ceiling(max(global(r_a, max, na.rm=T)$max)), 1:length(v_q)), ncol = 3)
    if(is.na(m[nrow(m),2])) m[nrow(m),2] <- m[nrow(m),1] + 1
	if(m[nrow(m),2] < m[nrow(m),1]) m[nrow(m),2] <- m[nrow(m),1] + 1
...
comp_lin_tot_mon <- function(pollutant, l_m_1, l_m_2, plot_dir){
  ...
  dt1 <- l_m_1[["summary"]] %>% .[, .(emis_kt = sum(emis_t, na.rm=T)/1000), by = .(Area, t)]
  dt1[, file := paste0(unique(l_m_1$summary$emis_y),"_",unique(l_m_1$summary$inv_y),"_",unique(l_m_1$summary$emep_v))]
  dt2 <- l_m_2[["summary"]] %>% .[, .(emis_kt = sum(emis_t, na.rm=T)/1000), by = .(Area, t)]
  dt2[, file := paste0(unique(l_m_2$summary$emis_y),"_",unique(l_m_2$summary$inv_y),"_",unique(l_m_2$summary$emep_v))]
...
comp_lin_sec_mon <- function(pollutant, l_m_1, l_m_2, plot_dir){
  ...
  dt1 <- l_m_1[["summary"]] %>% .[, .(emis_kt = sum(emis_t, na.rm=T)/1000), by = .(Area, GNFR, t)]
  dt1[, file := paste0(unique(l_m_1$summary$emis_y),"_",unique(l_m_1$summary$inv_y),"_",unique(l_m_1$summary$emep_v))]
  dt2 <- l_m_2[["summary"]] %>% .[, .(emis_kt = sum(emis_t, na.rm=T)/1000), by = .(Area, GNFR, t)]
  dt2[, file := paste0(unique(l_m_2$summary$emis_y),"_",unique(l_m_2$summary$inv_y),"_",unique(l_m_2$summary$emep_v))]
...
# list for plots
  ...
  g1 <- ggplot(data = dt[GNFR == i & Area != "SEA"], 
...
comp_bar_sec_ann <- function(pollutant, l_m_1, l_m_2, plot_dir){
  ...
  dt_tots_1 <- dt_tots_1[, .(emis_kt = sum(emis_t, na.rm=T)/1000), by = .(Area, GNFR, emis_y, inv_y, emep_v, plot_group)]

  dt_tots_2 <- l_m_2[["summary"]]
  dt_tots_2[, plot_group := paste0(emis_y,"_",inv_y,"_",emep_v)]
  dt_tots_2 <- dt_tots_2[, .(emis_kt = sum(emis_t, na.rm=T)/1000), by = .(Area, GNFR, emis_y, inv_y, emep_v, plot_group)]

  dt_tots <- rbindlist(list(dt_tots_1, dt_tots_2), use.names = T)


  p <- ggplot(dt_tots, aes(x = GNFR, y = emis_kt, group = plot_group, fill = plot_group))+
