###############################################################################
# HTML version of compare_functions.R. This keeps the existing plotting and data
# preparation workflow, but renders compare_markdown_html_v5.Rmd to HTML instead of
# requiring a TeX/PDF toolchain.
###############################################################################
packs <- c(
  "sf",
  "terra",
...
output_comparison_annual <- function(
  ...
  # Render the HTML report. The Rmd is HTML-only and contains no LaTeX/PDF
  # requirements, so it should work on systems where TeX is unavailable.
...
construct_filepath <- function(domain, pollutant, fol, y, map, time_res) {
  ...
  if (is.na(fol_emisInv)) {
    ...
    #)[, 3]
    # At the moment, global invs should have version in 6th position
    fol_emisInv <- strsplit(fol, "/")[[1]][6]
  } else if (length(fol_emisInv) == 1) {
    fol_emisInv <- gsub("inv", "", fol_emisInv)
    fol_emisInv <- gsub("/i", "", fol_emisInv)
  } else {
...
create_folders <- function(
  ...
  comp_root <- paste0(
    ...
    "_",
    l_fname_from$metadata$map_y,
    "_",
    l_fname_from$metadata$projectName,
    "_vs_",
    l_fname_to$metadata$data_source,
    "_",
    l_fname_to$metadata$inv_y,
    "_",
    l_fname_to$metadata$map_y,
    "_",
    l_fname_to$metadata$projectName,
    "/",
    time_res
  )

  plot_dir <- file.path(comp_root, "plots")
  table_dir <- file.path(comp_root, "tables")
...
worked_totals_annual <- function(pollutant, y, folname, nc_fname, dt_metadata) {
  ...
  tab_fol <- file.path(
    folname,
    "tables",
    paste0(dt_metadata[, emis_y], "_", dt_metadata[, map_y])
  )
...
comp_map_tot_ann <- function(
  ...
  if (l_fname_from$metadata$area == "GLOBAL") {
    ggsave(fname, p, width = 19, height = 15)
  } else {
...
comp_map_sec_ann <- function(
  ...
  for (i in 1:13) {
    ...
    if (global(r_a, sum, na.rm = T)[, 1] != 0) {
      ...
      # reclassify and extract the levels
      r_a_rc <- terra::classify(r_a, m, include.lowest = TRUE)
      r_a_rc <- as.factor(r_a_rc)
    ...
    if (l_fname_from$metadata$area == "GLOBAL") {
      ggsave(fname, p, width = 15.5, height = 13, limitsize = F)
    } else if (l_fname_from$metadata$area == "EU") {
      ggsave(fname, p, width = 12.5, height = 12, limitsize = F)
    } else {
...
domSNAP_map_tot_ann <- function(
  ...
  if (l_fname_from$metadata$area == "GLOBAL") {
    ggsave(fname, g_change, width = 15, height = 8, limitsize = F)
  } else {
