...
create_qaqc <- function(project, scenario, y, species, uk_folname, eu_folname, map_yr_uk, naei_inv, 
  ...
  gg_p8 <- eu_map_tot_ann(l_data_uk = l_uk_maps[["ann_tot_all"]], 
				  	      l_data_eu = l_eu_maps[["ann_tot_all"]],
					      y, species, eu_folname, emep_inv)
						  
  # 9. Bar plot; EU emissions processing check-plot
  l_gg_p9 <- eu_bar_inv_ann(fname_inv = l_fname_eu_sums[["inventory"]],
  ...
  gg_p10 <- eu_bar_nc_ann(fname_ncinp = l_fname_eu_sums[["ncinput"]], 
					      fname_ncout = l_fname_eu_sums[["ncoutput"]], 
					      y, species, eu_folname,
					      emep_inv, dt_emis = l_gg_p9[["table"]])
						 
  # 11. Line plot; EU emissions of total by month in netcdf
  # if(time_dim == "annual") ; produce a plot using the EMEP version profiles
  l_gg_p11 <- eu_lin_tot_mon(fname_ncout = l_fname_eu_sums[["ncoutput"]], 
                             y, species, eu_folname, 
  ...
  gg_p12 <- eu_lin_sec_mon(dt_month = l_gg_p11[["table"]], y,
                           species, eu_folname, 
						   emep_inv, time_dim, emep_version)
						   
  # 13. Maps; EU emissions, sector total cell-1 annum-1.
  gg_p13 <- eu_map_sec_ann(l_data_uk = l_uk_maps[["ann_tot_sec"]], 
                           l_data_eu = l_eu_maps[["ann_tot_sec"]], 
		  			 	   y, species, eu_folname, 
						   map_yr_uk, naei_inv, emep_inv)					   
...
eu_map_tot_ann <- function(l_data_uk, l_data_eu, y, species, 
                           eu_folname, emep_inv){
  ...
  fname <- paste0(eu_folname,"/plots/e",y,"/",
                  dt_poll[ceh_poll == species, emep_model],"_EU_",
				  y,"emis_",y,"map_",emep_inv,"inv_8_EUTOTMAP.png")
...
eu_bar_inv_ann <- function(fname_inv, fname_proc, y, species, 
                           eu_folname, emep_inv){

  # summarise the inventory information.
  dt_inv <- fread(fname_inv)
  setnames(dt_inv, c("Data_source", "Year", "EMEP_data"), 
                   c("data_source", "emis_y", "inv_y"))
  
  dt_inv_t <- dt_inv[, lapply(.SD, sum, na.rm=T), by = .(ISO, Pollutant, 
  ...
  setnames(dt_proc, c("Data_source", "Year"), 
                    c("data_source", "emis_y"))
  dt_proc[, inv_y := emep_inv]
  dt_proc[, emis_t := ann_emis_kt * 1000]
				   
  dt_proc_t <- dt_proc[, lapply(.SD, sum, na.rm=T), by = .(ISO, 
  ...
  dt[, stage := factor(stage, levels = c("inventory_data",
                                         "processed_data"))]
  #dtw <- dcast(dt, ISO+Pollutant~stage, value.var = "emis_t")
  #dtw[, kept := processed_data / inventory_data]
  
  # labels text
  dt_text <- data.table(stage = rep(c("inventory_data",
                                      "processed_data")),
						emis_t = c(dt[, sum(emis_t), 
						             by = .(stage)][,V1]),	
						label = as.character(round(dt[, sum(emis_t)/1000, 
  ...
  # plot
  p <- ggplot()+
    geom_bar(data = dt, aes(x = stage, y = emis_t/1000, group = ISO, fill= ISO),
       	     stat = "identity")+
	# scale_fill_manual(values = c("#eecea6","#59aed3","#6fbe6d","#ea93ea"))+
    labs(y = bquote(kt~a^-1))+	
    # facet_wrap(~Area, nrow=1, scales = "free_y")+
  ...
  fname <- paste0(eu_folname,"/plots/e",y,"/",
                  dt_poll[ceh_poll == species, emep_model],"_EU_",
				  y,"emis_",y,"map_",emep_inv,"inv_9_EUINVBAR.png")
...
eu_bar_nc_ann <- function(fname_ncinp, fname_ncout, 
					      y, species, eu_folname,
					      emep_inv, dt_emis){
  ...
  dt_ncinp_t <- dt_ncinp[, lapply(.SD, sum, na.rm=T), by = .(ISO, Pollutant, 
                                  data_source, emis_y, inv_y), 
						          .SDcols = c("emis_t_tot_ncinput",
								              "emis_t_tot_array","tsum")]
  ...
  dt_ncinp_m <- melt(dt_ncinp_t, id.vars = c("ISO","Pollutant",
                                             "data_source","emis_y","inv_y"),
				     variable.name = "stage", value.name = "emis_t")
  ...
  dt_ncout_m <- melt(dt_ncout_t, id.vars = c("ISO","Pollutant",
                                             "data_source","emis_y","inv_y"),
				     variable.name = "stage", value.name = "emis_t")
  ...
  dt[, stage := factor(stage, levels = c("processed_data", "tot_ncinput",
										 "tot_array", "time_layers", 
										 "tot_ncoutput"))]
	  
  # labels text
  dt_text <- data.table(stage = c("processed_data", "tot_ncinput",
								  "tot_array", "time_layers", 
								  "tot_ncoutput"),
						emis_t = c(dt[, sum(emis_t), 
						             by = .(stage)][,V1]),	
  ...
  # plot
  p <- ggplot()+
    geom_bar(data = dt, aes(x = stage, y = emis_t/1000, fill = Pollutant), 
	         stat = "identity")+	
  ...
  fname <- paste0(eu_folname, "/plots/e", y, "/",
                  dt_poll[ceh_poll == species, emep_model], "_EU_",
				  y, "emis_", y, "map_", emep_inv, "inv_10_EUNCBAR.png")
...
eu_lin_tot_mon <- function(fname_ncout, y, species, eu_folname, 
				  	         emep_inv, time_dim, emep_version){
  ...
  dt_out_EU <- dt_out_sum[, list(emis_kt = sum(emis_kt, na.rm=T)), 
                          by = .(data_source, time, line_type)]
  dt_out_EU[, ISO := "EU"]
  dt_out_iso <- dt_out_sum[ISO %in% c("NOS", "ATL", "BE", 
                                      "FR", "NL", "ES", "DE")]
    
  # overwrite dt_sum_out;
  dt_out_sum <- rbindlist(list(dt_out_EU, dt_out_iso), use.names = T)
  dt_out_sum[, ISO := factor(ISO, levels = c("EU", "FR", "NL", "BE", "DE", 
                                             "ES", "NOS", "ATL"))]
  ...
  fname <- paste0(eu_folname,"/plots/e",y,"/",
				  dt_poll[ceh_poll == species, emep_model],"_EU_",
				  y,"emis_",y,"map_",emep_inv,"inv_11_EUMONTOTLINE.png")
...
eu_lin_sec_mon <- function(dt_month, y, species, eu_folname, 
						   emep_inv, time_dim, emep_version){
  ...
  # plot info
  dt_month <- dt_month[, list(emis_kt = sum(emis_t, na.rm=T)/1000),
  ...
  fname <- paste0(eu_folname,"/plots/e",y,"/",
				  dt_poll[ceh_poll == species, emep_model],"_EU_",
				  y,"emis_",y,"map_",emep_inv,"inv_12_EUMONSECLINE.png")
...
eu_map_sec_ann <- function(l_data_uk, l_data_eu, 
		  			 	   y, species, eu_folname, 
						   map_yr_uk, naei_inv, emep_inv){
  ...
  m <- matrix(c(0, v_q[1:7], v_q[1:7], 
              ...
              1:8), ncol = 3) 
  brew_d = -1
  v_labs <- unlist(lapply(1:nrow(m), function(x) paste0(round(m[x,1],3), " - ",
                                                      round(m[x,2], 3))))
  ...
  fname <- paste0(eu_folname,"/plots/e",y,"/",
                  dt_poll[ceh_poll == species, emep_model],"_EU_",
				  y,"emis_",y,"map_",emep_inv,"inv_13_EUANNSECMAP.png")
...
eu_map_tot_mon <- function(dt_month, l_eu_maps, y, 
                           species, eu_folname,
						   emep_inv, time_dim, emep_version){
  ...
  if(time_dim == "annual"){
	...
	dt_iso_mon <- dt_month[, list(emis_kt = sum(emis_t, na.rm=T)/1000), 
	                         by = .(time)]
    # index the emissions
