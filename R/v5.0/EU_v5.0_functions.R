...
EMEP_EU_v5.0 <- function(
  ...
  if (!is.numeric(emep_inv)) {
    stop("Reporting year is not numeric")
  }

  print(paste0(
    format(Sys.time(), "%F %T"),
    ": Creating EMEP4UK EU inputs (",
    time_dim,
    ") for ",
    y,
    "..."
  ...
  for (species in v_pollutants) {
    ...
    for (i in v_sectors) {
      ...
      dt_totals <- summarise_EU_emissions(
        y,
        species,
        i,
        l_s = l_eu_prof,
    ...
    dt_ncinput_summary <- input_data_NETCDF_eu(
      y,
      species,
      emep_inv,
      time_dim,
    ...
    dt_ncoutput_summary <- summarise_nc_file_eu(
      fname_ncdf,
      y,
      species,
      emep_inv,
...
ISO_sector_raster <- function(iso, dt_emis, i, species, y, UKmask) {
  ...
  if (y < 1990) {
    ...
    dt_ceds_90 <- dt_ceds_data[
      Year == 1990 & GNFR == dt_sec[sec == i, GNFRlong]
    ] # this is for tricky ISOs (below)
    ...
    if (i == "sec07") {
      ...
      if (iso %in% v_CEDS_missing) {
        ...
        alpha_fac <- dt_ceds_data[
          Year == y & GNFR == "P_IntShipping" & ISO2 == "XX",
          alpha
        ]
      } else {
      ...
      if (iso %in% v_CEDS_missing) {
        ...
        all_iso_tot_y <- dt_ceds[
          ISO2 %in% dt_emis[, unique(ISO2)],
          sum(emis_kt, na.rm = T)
        ]
        all_iso_tot_90 <- dt_ceds_90[
          ISO2 %in% dt_emis[, unique(ISO2)],
          sum(emis_kt, na.rm = T)
        ]
    ...
    if (i != "sec09") {
      ...
      # it is important that I_Offroad is treated separately; using actual kt value
...
input_data_NETCDF_eu <- function(
  ...
  if (
    unique(unlist(lapply(l_eu, function(x) length(x)))) != length(v_EMEP_sec)
  ) {
