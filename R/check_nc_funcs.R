##############################################################################################################
packs <- c("sf","terra","stringr","dplyr","ggplot2","data.table","stats","readxl","ncdf4","lubridate","ggrepel","patchwork")

lapply(packs, require, character.only = TRUE)
##############################################################################################################

#### MASTER function to call needed functions based on choices
evaluateNC <- function(species, v_years, naei_inv, map_yr_uk, map_yr_ie, emep_inv, tp_scheme = "genYr", dt_sec,
		               summarise_UK = FALSE, plot_UK = FALSE, summarise_EIRE = FALSE,
...
evaluateUKnc <- function(species, v_years, naei_inv, map_yr_uk, tp_scheme, dt_sec){
  ...
  for(y in v_years){
    ...
    output_dir <- paste0("/gws/nopw/j04/ceh_generic/samtom/EMEP_inputs/outputs/EMEP4UK/inv",naei_inv,"/emis",y,"/UKEIRE/TP",tp_scheme,"_AGGNA")
  
    nc_file <- paste0(output_dir,"/",species,"_UKEIRE_",y,"emis_",map_yr_uk,"map_",naei_inv,"inv_0.01.nc")
	...
	for(v in nc_secs){
	  ...
	  if(grepl(paste(v_secs_skip, collapse = "|"), v)) next
	  ...
	  dt <- data.table(Pollutant = species, Area = str_split(v, "_")[[1]][2], Year = y, Month = 1:12, GNFR = GNFR_name, 
	                   Sector = layer_name, layer = names(r), emis_kt = global(r, sum, na.rm=T)$sum/1000)
...
plotUKnc <- function(species, naei_inv, map_yr_uk, tp_scheme){
  ...
  dt_ncGNFRsum <- dt[,.(emis_kt = sum(emis_kt, na.rm=T)), by = .(Pollutant, Area, Year, GNFR, Sector) ] %>% .[, source := "UK"]
  dt_ncSNAPsum <- dt[,.(emis_kt = sum(emis_kt, na.rm=T)), by = .(Pollutant, Area, Year, SNAP) ] %>% .[, source := "UK"]
  ...
  if(species == "voc") dt_nc_info[, Pollutant := "voc"]
  
  dt_nc_info <- dt_nc_info[!(sec_long %in% c("sec14","sec15","sec16","sec17","sec18","sec19")) & Area == "uk" & Data_source == "Emissions_files"]
  
  dt_nc_info[, c("Area","GNFR","EMEP_Sector","sec_long","long_name","Data_source","Incoming_annual_kt","Terres_annual_kt","Terres_10_annual_kt",paste0("Terres_M",1:12),"Terres_ann") := NULL]
  setnames(dt_nc_info, c("SEA_annual_kt","Out_of_Mask_annual_kt"), c("SEA","OOM"))
  
  dtm_nc <- melt(dt_nc_info, id.vars = c("Pollutant","SNAP","Year"), variable.name = "Area", variable.factor = F, value.name = "emis_kt")
  dtm_nc <- dtm_nc[,.(emis_kt = sum(emis_kt, na.rm=T)), by = .(Pollutant, Area, Year, SNAP)] %>% .[, source := "x"]
  
  dt_terres_sea <- rbindlist(list(dt_ncSNAPsum, dtm_nc[Area == "SEA"]), use.names=T) %>% .[, source := "SEA"]
  dt_terres_sea <- dt_terres_sea[,.(emis_kt = sum(emis_kt, na.rm=T)), by = .(Pollutant, Year, SNAP, source)]
  dt_terres_sea[, Area := "SEA"]
  
  
  dt_terres_sea_oom <- rbindlist(list(dt_ncSNAPsum, dtm_nc), use.names=T) %>% .[, source := "ALL"]
  dt_terres_sea_oom <- dt_terres_sea_oom[,.(emis_kt = sum(emis_kt, na.rm=T)), by = .(Pollutant, Year, SNAP, source)]
  dt_terres_sea_oom[, Area := "ALL"]
  
  dt_point_data <- rbindlist(list(dt_ncSNAPsum, dt_terres_sea, dt_terres_sea_oom), use.names = T)
  setnames(dt_point_data, "Area", "AREA")
  
  # make a totals table, for the totals line graph
  dt_nc_TOT_TER <- dt[, .(emis_kt = sum(emis_kt, na.rm=T)), by = .(Pollutant, Year, Area) ]
  setnames(dt_nc_TOT_TER, "Area", "AREA")
  dt_nc_TOT_TER[, source := "UK"]
  dt_nc_TOT_TER[, lt := "A"]
  
  dt_nc_TOT_TERSEA <- dt_terres_sea[, .(emis_kt = sum(emis_kt, na.rm=T)), by = .(Pollutant, Year, Area) ]
  setnames(dt_nc_TOT_TERSEA, "Area", "AREA")
  dt_nc_TOT_TERSEA[, source := "SEA"]
  dt_nc_TOT_TERSEA[, lt := "B"]
  
  dt_nc_TOT_TERSEADOM <- dt_terres_sea_oom[, .(emis_kt = sum(emis_kt, na.rm=T)), by = .(Pollutant, Year, Area) ]
  ...
  dt_ship <- dt_ship[!(sec_long %in% c("sec14","sec15","sec16","sec17","sec18","sec19")) & AREA == "uk" & Data_source == "Emissions_files"]
  
  dt_ship <- dt_ship[SNAP == 8, c("Pollutant","Year","SNAP","Out_of_Mask_annual_kt","AREA")]
  dt_ship <- dt_ship[, .(ship_kt = sum(Out_of_Mask_annual_kt, na.rm=T)), by = .(Pollutant, Year, SNAP, AREA)]
  
  dt_ship <- dt_nc_TOT_TERSEADOM[dt_ship, on = c("Pollutant","Year")][, c("Pollutant","Year", "emis_kt", "ship_kt")] %>% setnames(., "emis_kt", "all_kt")
  dt_ship <- dt_nc_TOT_TERSEA[dt_ship, on = c("Pollutant","Year")][, c("Pollutant","Year", "emis_kt", "all_kt", "ship_kt")] %>% setnames(., "emis_kt", "sea_kt")
  ...
  dt_NAEI_SNAP <- fread(paste0("../../inventory_processor/data/NAEI/inv",naei_inv,"/alpha/NAEI_AllPoll_TOTALS_inv",
                               naei_inv,"_emis_1970-",naei_inv-2,"_SNAP_alpha.csv"))[AREA == "UK"]
  if(species != "voc"){
  ...
  setnames(dt_NAEI_SNAP, "tot_emis_t", "emis_t") ; dt_NAEI_SNAP[, "tot_alpha" := NULL]
  dt_NAEI_SNAP[, source := "INVENTORY"]
  
  dt_NAEI_GNFR <- fread(paste0("../../inventory_processor/data/NAEI/inv",naei_inv,"/alpha/NAEI_AllPoll_TOTALS_inv",
                               naei_inv,"_emis_1970-",naei_inv-2,"_GNFR_alpha.csv"))[AREA == "UK"]
  if(species != "voc"){
  ...
  setnames(dt_NAEI_GNFR, "tot_emis_t", "emis_t") ; dt_NAEI_GNFR[, "tot_alpha" := NULL]
  dt_NAEI_GNFR[, source := "INVENTORY"]
  
  # time series of point data from NAEI
  dt_pts_NAEI_SNAP <- fread(paste0("../../inventory_processor/data/NAEI/inv",naei_inv,"/points/NAEI_AllPoll_POINTS_inv",
                                   naei_inv,"_emis_1990_",naei_inv-2,"_SNAP_t_LL.csv"))[AREA == "UK"]
  if(species != "voc"){
  ...
  dt_pts_NAEI_SNAP <- dt_pts_NAEI_SNAP[, c("Pollutant", "Year","SNAP","AREA","emis_t")]
  dt_pts_NAEI_SNAP[, source := "POINTS_NAEI"]
  
  dt_pts_NAEI_GNFR <- fread(paste0("../../inventory_processor/data/NAEI/inv",naei_inv,"/points/NAEI_AllPoll_POINTS_inv",
                                   naei_inv,"_emis_1990_",naei_inv-2,"_GNFR_t_LL.csv"))[AREA == "UK"]
  if(species != "voc"){
  ...
  dt_pts_NAEI_GNFR <- dt_pts_NAEI_GNFR[, c("Pollutant", "Year","GNFR","AREA","emis_t")]
  ...
  dt_SPEED <- fread(paste0("../SPEED/SPEED_AllPoll_TOTALS_invNA_emis_1960-1970_SNAP_alpha.csv"))
  dt_SPEED <- dt_SPEED[Pollutant == species]
  setnames(dt_SPEED,"tot_emis_t","emis_t")
  
  dt_pts_SPEED_SNAP <- dt_SPEED[, c("Pollutant", "Year","SNAP","AREA","emis_t")]
  ...
  # bind and plot
  dt_SNAP_data <- rbindlist(list(dt_NAEI_SNAP, dt_pts_NAEI_SNAP, dt_pts_SPEED_SNAP), use.names = T)
  dt_SNAP_data <- dt_SNAP_data[, .(emis_kt = sum(emis_t, na.rm=T)/1000), by = .(Pollutant, Year, SNAP, AREA, source)]
  if(species == "voc") dt_SNAP_data[, Pollutant := "voc"]
  ...
  dt_NAEI_no99 <- dt_NAEI_SNAP[SNAP != 99, .(emis_kt = sum(emis_t, na.rm=T)/1000), by = .(Pollutant, Year, AREA, source)] %>% .[, lt := "D"]
  dt_NAEI_99 <- dt_NAEI_SNAP[, .(emis_kt = sum(emis_t, na.rm=T)/1000), by = .(Pollutant, Year, AREA, source)] %>% .[, c("source","lt") := list("INVENTORY+INT.AVI","E")]
  
  dt_plot_TOTS <- rbindlist(list(dt_NAEI_no99 , dt_NAEI_99, dt_nc_TOT_TER, dt_nc_TOT_TERSEA, dt_nc_TOT_TERSEADOM), use.names = T)
  if(species == "voc") dt_plot_TOTS[, Pollutant := "voc"]
  ...
  if(species == "nox"){
   ...
   g_SNAP <- ggplot()+
    geom_line(data = dt_SNAP_data, aes(x = Year, y = emis_kt, group = source, colour = source))+
    geom_point(data = dt_point_data, aes(x = Year, y = emis_kt, group = source), colour = "#23CD38", size = 2)+
  ...
  dt_plot_TOTS_labs <- dt_plot_TOTS_labs[Year <= 2010]
   ...
   g_TOTS <- ggplot()+
    geom_line(data = dt_plot_TOTS, aes(x = Year, y = emis_kt, group = lt, colour = lt, linetype = lt))+
   ...
   p1 <- p1 + plot_annotation(
     title = paste0(species, ' emissions in the UK-NAEI inventory and EMEP4UK input files (green dots)'),
...
plotEIREnc <- function(species, emep_inv, tp_scheme){
  ...
  dt_ncGNFRsum <- dt[,.(emis_kt = sum(emis_kt, na.rm=T)), by = .(Pollutant, Area, Year, GNFR, Sector) ] %>% .[, source := "EIRE"]
    
  ########################
  #### full EMEP data ####
  ########################
  
  dt_EMEP_GNFR <- fread(paste0("../../inventory_processor/data/EMEP/inv",emep_inv,"/alpha/EMEP_AllPoll_TOTALS_inv",
                               emep_inv,"_emis_1970-",emep_inv-2,"_GNFR_alpha.csv"))[ISO2 == "IE"]
  if(species != "voc"){
  ...
  #####################
  if(species != "voc"){
    dt_CEDS_GNFR <- fread(paste0("../SPEED/CEDS_for_EMEP/",species,"_CEDS_1950_1990_ISO_GNFR_kt.csv"))[ISO2 == "IE"]
  }else{
    dt_CEDS_GNFR <- fread(paste0("../SPEED/CEDS_for_EMEP/nmvoc_CEDS_1950_1990_ISO_GNFR_kt.csv"))[ISO2 == "IE"]
  }
  ...
  dt_CEDS_reltemp <- dt_EMEP_GNFR[dt_CEDS_reltemp, on = c("Pollutant","ISO2","Year","GNFR")]
  dt_CEDS_reltemp[, emis_scal := emis_kt[Year == 1990] * alpha, by = GNFR]
  
  dt_CEDS_rel <- dt_CEDS_reltemp[, c("Year","ISO2","GNFR","emis_scal","Pollutant")] %>% .[, source := "CEDS_rel"]
  setnames(dt_CEDS_rel, "emis_scal", "emis_kt")
  
  # bind and plot
  dt_GNFR_data <- rbindlist(list(dt_EMEP_GNFR, dt_CEDS_act, dt_CEDS_rel), use.names = T)
  if(species == "voc") dt_GNFR_data[, Pollutant := "voc"]
  
  ## TOTALS
  dt1 <- dt_ncGNFRsum[, .(emis_kt = sum(emis_kt, na.rm=T)), by = .(Year, source) ]
  dt2 <- dt_GNFR_data[, .(emis_kt = sum(emis_kt, na.rm=T)), by = .(Year, source) ]
  ...
  #}
   ...
   g_GNFR <- ggplot()+
    geom_line(data = dt_GNFR_data, aes(x = Year, y = emis_kt, group = source, colour = source))+
    geom_point(data = dt_ncGNFRsum, aes(x = Year, y = emis_kt, group = source), colour = "#23CD38", size = 2)+
	#geom_line(data = dt_point_data[SNAP %in% v_gp_SN], aes(x = Year, y = emis_kt, group = source), colour = "#23CD38", alpha=0.4)+
    #geom_text_repel(data = dt_point_data[SNAP %in% v_gp_SN], min.segment.length = 0, aes(x = Year, y = emis_kt, label = source), size = 3.5)+
    #geom_label(data = dt_ind_lab, aes(label = lab), x = 2001, y = ind_lab_y, hjust = 0, size = 3)+
    scale_colour_manual(values = c("black","blue","red"))+
  ...
  #}
   ...
   g_TOTS <- ggplot()+
    geom_line(data = dt_TOTS, aes(x = Year, y = emis_kt, group = source, colour = source))+
	#geom_line(data = dt_plot_TOTS[source %in% c("UK","SEA","ALL")], aes(x = Year, y = emis_kt, group = lt, colour = lt), alpha=0.4)+
    #geom_point(data = dt_plot_TOTS[source %in% c("UK","SEA","ALL")], aes(x = Year, y = emis_kt, group = lt, colour = lt), size = 3)+
    #{if(species %in% c("nox","sox","pmco","pm25","voc"))geom_text_repel(data = dt_plot_TOTS_labs, min.segment.length = 0, aes(x = Year, y = emis_kt, label = source), size = 4)}+
    #scale_colour_manual(name = "source", values = c("D" = "black","E" = "#078CBE","A" = "#23CD38","B" = "#23CD38","C" = "#23CD38"), breaks = c("D","E"), labels = c("INVENTORY","INVENTORY+INT.AVI"))+
   ...
   p1 <- p1 + plot_annotation(
     title = paste0(species, ' emissions in EIRE in the EMEP & CEDS inventory and EMEP4UK input files'),
     subtitle = 'UK = terrestrial emissions ; SEA = UK + 10km sea buffer ; ALL = all emissions, including those excluded to outer domain') & theme(plot.title = element_text(size = 24),
                        plot.subtitle = element_text(size = 18))
...
evaluateEUnc <- function(species, v_years, emep_inv, tp_scheme, dt_sec){
  ...
  for(y in v_years){
    ...
    ## choose output directory for the EMEP input files
    output_dir <- paste0("/gws/nopw/j04/ceh_generic/samtom/EMEP_inputs/outputs/EMEP4UK/inv",naei_inv,"/emis",y,"/EU/TP",tp_scheme,"_AGGoneEU")
  
    nc_file <- paste0(output_dir,"/",species,"_EU_",y,"emis_",y,"map_",emep_inv,"inv_0.1.nc")
	...
	for(v in nc_secs){
	  ...
	  dtm_zonal <- melt(dt_zonal, id.vars = c("EMEP_code"), variable.name = "layer", variable.factor = F, value.name = "emis_t")
	  dtm_zonal <- dt_iso[dtm_zonal, on = c("EMEP_code")]
	  
	  # extra info
	  dtm_zonal[, "Month" := tstrsplit(layer, "_", keep = 4)] %>% .[, Month := as.numeric(Month)]
	  dtm_zonal[, c("Pollutant","Year","GNFR","Sector","emis_kt") := list(species, y, GNFR_name, layer_name, emis_t/1000)]
...
plotEUnc <- function(species, emep_inv, tp_scheme){
  ...
  # format the NC summary data
  dt <- fread(paste0("outputs/EMEP4UK/inv2023/plots/EU/",species,"_EU_nc_data.csv"))

  dt_EU <- dt[, .(emis_kt = sum(emis_kt, na.rm=T)), by = .(layer,Month,Pollutant,Year,GNFR,Sector)]
  dt_EU[, c("EMEP_iso","EMEP_code","Name") := list("XX",NA,"EMEP_domain")]

  # aggregate nc to GNFR and SNAP
  dt_GNFRsum <- dt[,.(emis_kt = sum(emis_kt, na.rm=T)), by = .(EMEP_iso, EMEP_code, Name, Pollutant, Year, GNFR, Sector) ]
  dt_EU_GNFRsum <- dt_EU[,.(emis_kt = sum(emis_kt, na.rm=T)), by = .(EMEP_iso, EMEP_code, Name, Pollutant, Year, GNFR, Sector) ]
  ...
  if(species != "voc"){
    ...
    dt_ceds_data <- fread(paste0("../SPEED/CEDS_for_EMEP/nmvoc_CEDS_1950_1990_ISO_GNFR_kt.csv"))
  }
  
  dt_ceds   <- dt_ceds_data[Year < 1990 & ISO2 != ""] # this is for tricky ISOs (below)
  dt_ceds   <- dt_iso[dt_ceds, on = "ISO2"] %>% setnames(., "EMEP_ISO", "EMEP_iso")
