...
splitUKIEannual <- function(species, time_dim = c("year","month"), tp_scheme, l_annual, i, country = c("uk","ie","sea")){
  ...
  if(time_dim == "year"){
	...
	## If the tp_scheme = "pre_TEMREG";
	     # this is EMEP4UKv4.45 temporal data in July 2023. Monthly splits, for 5 SNAPs, per ISO
    ## HOWEVER if the tp_scheme != "pre_TEMREG";
	...
	# IF the tp_scheme = "pre_TEMREG", use the previous EMEP defaults;
	  
	  country_id <- ifelse(country == "uk", 27, 14) # to extract correct femis factors; 27 = UK, 14 = IE (Eire)
      snap_id <- dt_sec[sec == i, as.numeric(SNAP)] # set the SNAP to read from the femis. 
    
      # read in femis file for legacy temporal splits (subset to Eire or UK - SEA needs to match parent country)
      if(species == "nmvoc"){
	    dt_femis <- fread(paste0("/gws/nopw/j04/ceh_generic/samtom/TEMREG/output/model_inputs/EMEP4UK/",tp_scheme,"/MonthlyFacs.voc"))
	  }else{
	    dt_femis <- fread(paste0("/gws/nopw/j04/ceh_generic/samtom/TEMREG/output/model_inputs/EMEP4UK/",tp_scheme,"/MonthlyFacs.",species))
	  }
      ...
      if(is.na(snap_id)){
        v_femis <- rep(1,12)
      }else if(snap_id %in% dt_femis_m[,SNAP]){
	    v_femis <- dt_femis_m[ISO == country_id & SNAP == snap_id][["FAC"]] # vector of monthly splits 
	    v_femis <- v_femis/mean(v_femis)# not always adding to 12 in the femis file, adjust slightly
	  }else{
	    v_femis <- rep(1,12)	          
      }	
	...
	# GAMs can produce -ve values in profiles which will cause -ve emissions
	  ...
	  # IF it is M_Other, set to 1s
		...
		if(country == "uk"){
		  ...
		  dt_femis <- fread(paste0("/gws/nopw/j04/ceh_generic/samtom/TEMREG/output/coeff_sector/SNAP/",species,"/",tp_scheme,
							   "/SNAP_",dt_sec[sec == i, str_pad(SNAP, 2, "0", side = "left")],"_month_",species,"_",tp_scheme,".csv"))
	  
	      v_femis <- dt_femis[,N] # vector of monthly splits 
		  v_femis[v_femis < 0] <- 0.02 # rare occasion factor goes below 0, set to 0.02 (i.e. some emissions, but tiny)
	      v_femis <- v_femis/mean(v_femis)# not always adding to 12 in the femis file, adjust slightly
		
		}else{
          		
		  dt_femis <- fread(paste0("/gws/nopw/j04/ceh_generic/samtom/TEMREG/output/coeff_sector/GNFR/",species,"/",tp_scheme,
